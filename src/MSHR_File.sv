...
// Dispacher expands completed line bundles into miss responses.
// ============================================================
...
module MSHR_File #(
    ...
    logic dispacher_ready;

    logic rs_retire_valid;
    logic [LINE_ADDR_WIDTH-1:0] rs_retire_line_addr;
    logic [LINE_WIDTH-1:0]      rs_retire_fill_line;

    logic dispatch_valid;
    logic [LINE_WIDTH-1:0]      dispatch_fill_line;
    ...
    Reservation_Station #(
        ...
        .retire_fill_line   (rs_retire_fill_line),

        .dispatch_valid        (dispatch_valid),
        .dispatch_ready        (dispacher_ready),
        .dispatch_fill_line    (dispatch_fill_line),
    ...
    Dispacher #(
        ...
        .dispatch_valid       (dispatch_valid),
        .dispacher_ready      (dispacher_ready),
    ...
    assign rs_retire_fill_line = entry_fill_line[retire_sel_idx];
    ...
    // ============================================================
    // Refill mux
    // Still needed for cache array refill/writeback install path.
    // ============================================================
    ...
    assign full  = rs_almost_full;
    assign empty = rs_empty && ~|entry_valid;
    ...
    always_ff @(posedge clk) begin
        if (!rst) begin
            ...
            if (rs_retire_valid && DEBUG) begin
                $display("[%0t] MSHR_FILE_RETIRE_SELECT: mshr=%0d line_addr=%h fill_line=%h",
                         $time,
                         retire_sel_idx,
                         rs_retire_line_addr,
                         rs_retire_fill_line);
